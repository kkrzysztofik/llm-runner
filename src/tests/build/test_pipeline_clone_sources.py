...
class TestBuildEnvCmd:
    ...
    @pytest.mark.parametrize(
        ("backend", "setvars_path", "expect_wrap"),
        [
            (BuildBackend.CUDA, "setvars.sh", False),
            (BuildBackend.SYCL, "nonexistent", False),
        ],
    )
    def test_get_build_env_cmd_no_wrap(
        ...
        fake_setvars = tmp_path / setvars_path
        if setvars_path != "nonexistent":
            fake_setvars.write_text("# mock")
            patch_target = fake_setvars
        else:
...
class TestRunFinalize:
    ...
    def _make_success_progress(self) -> BuildProgress:
        return BuildProgress(
            stage="build", status="success", message="build ok", progress_percent=100.0
        )
    ...
    def test_returns_none_when_build_incomplete(self, tmp_path: Path) -> None:
        ...
        progress = BuildProgress(
            stage="build", status="running", message="ok", progress_percent=50.0
        )
    ...
    def test_returns_none_when_status_not_success(self, tmp_path: Path) -> None:
        ...
        ctx = self._make_ctx(tmp_path)
        progress = BuildProgress(
            stage="build", status="failed", message="error", progress_percent=0.0
        )
        result = run_finalize(ctx, progress)
        assert result is None
