...
def _build_mock_config(tmp_path: Path, cuda_exists: bool = False) -> tuple[MagicMock, str, Path]:
    ...
    with patch("llama_cli.profile_cli.Config") as mock_cfg_cls:
        ...
        return mock_cfg_cls.return_value, str(bench_bin), profiles_dir
...
class TestCmdProfile:
    ...
    def test_success_path(
        ...
        mock_cfg, bench_bin, profiles_dir = _build_mock_config(tmp_path)
        ...
        with (
            ...
            patch(
                "llama_cli.profile_cli.build_benchmark_cmd", return_value=["bench", "-m", "model"]
            ),
    ...
    def test_cuda_backend_detection_in_profile(
        ...
        _build_mock_config(tmp_path, cuda_exists=True)
