...
- **FR-010**: The system MUST perform graceful shutdown via SIGTERM with a wait-and-escalate policy; abnormal TUI exit MUST terminate child servers to avoid orphan GPU processes. After sending SIGKILL, the system polls for process termination. If the process is still alive after a 2-second poll, the system logs a CRITICAL error, marks the slot as `offline`, and records the failure. The system continues shutting down remaining slots and reports all failures in the audit log. After all slots have been processed, the tool exits with code 130 if any slot required SIGKILL escalation. This indicates a kernel-level issue where the process is stuck in uninterruptible sleep (D state). Exit code 130 is reserved for process-manager SIGKILL escalation failures and is outside the doctor (1-9) and smoke (10-19) families.
...
| --- | --- | --- |
| `smoke_inter_slot_delay_s` | 2 | Pause between successful completion of slot *n* and start of slot *n+1* |
| `smoke_listen_timeout_s` | 30 | Timeout for smoke probing TCP ready check |
| `smoke_http_request_timeout_s` | 10 | HTTP request timeout for `/v1/models` and chat completion endpoints |
