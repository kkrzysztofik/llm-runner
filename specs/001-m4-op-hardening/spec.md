...
_Spec FR numbers are independent of PRD FR numbers. The mapping below is an **approximate reference** for traceability only — it does not imply a strict one-to-one correspondence. For implementation, always refer to the spec FR text as the authoritative requirement._
PRD→Spec reference mapping: FR-010→spec FR-001; FR-011→spec FR-019; FR-012→spec FR-010; FR-014→spec FR-007–009; FR-015→spec FR-002–006; FR-016→spec FR-013–014; FR-017→spec FR-011–012; FR-018→spec FR-016.
...
- **FR-002**: The system MUST provide `smoke both` and `smoke slot <slot_id>` commands that probe OpenAI-compatible endpoints sequentially.
  - **FR-002a** (command structure): `smoke both` probes all configured slots sequentially in declaration order from config, starting with the first slot and proceeding sequentially. `smoke slot <slot_id>` probes a single slot by ID.
  - **FR-002b** (inter-slot delay): A configurable pause is applied **after** each successful slot completion (not before the first slot). The default inter-slot delay is 2 seconds. The field is defined as `smoke_inter_slot_delay_s: int = 2` in the `Config` dataclass at `src/llama_manager/config.py` and read via factory functions in `src/llama_manager/config_builder.py`. On the CLI, `--delay` overrides this value.
  - **FR-002c** (CLI smoke — probe independence from TUI state): When invoked via CLI (`llm-runner smoke both` or `smoke slot <id>`), the probe operates independently of TUI slot state — it probes by port directly (attempting TCP connection). The CLI probe does NOT wait for a `launching` transition; it attempts the TCP connection and times out per `smoke.listen_timeout_s` if the server isn't ready.
  - **FR-002d** (TUI-context smoke — launching state wait): When smoke is invoked from the TUI monitor context (e.g., a user-triggered probe from the TUI UI), the probe waits up to `tui_launch_timeout_s` (default 120 s) if a slot is in the `launching` state. If the slot transitions to `running` during the wait, the probe proceeds to the next phase. If it transitions to `crashed`, the probe fails with exit code 19. The TUI-context wait does NOT apply to CLI smoke (see FR-002c).
  - **FR-002e** (no retries, composite reporting): Each smoke probe phase is attempted exactly once — no retries. Failure at any phase is immediate and non-recoverable for that probe session. If a slot fails, probing continues to the next slot in order; the command exits non-zero if any slot fails, producing a composite report.
...
- **FR-006**: If two consecutive smokes for the same slot fail with model-not-found attributable to a wrong ID, the system MUST require an explicit `smoke.model_id` override before further attempts. The 'two consecutive failures' counter is scoped to a single `llm-runner` process lifetime (in-memory only, not persisted to disk). A process restart resets the counter. This prevents requiring an override after a transient failure followed by a restart. 'Consecutive' means: only exit code 13 increments the counter; any other exit code (timeout, auth failure, connection error) does NOT increment it, but also does NOT reset it — the counter retains its value. If the user provides a `smoke.model_id` override that fails to match on the next attempt, the counter is NOT reset. The user must provide a correct override; otherwise the counter continues incrementing and the override requirement remains. At three consecutive failures the system MUST hard-block further smoke attempts for that slot and print a diagnostic message naming the expected model ID (from GGUF metadata or catalog), the last resolved model ID returned by the server, and the path to the model file. The user must then either provide a correct `--model-id` override or remove the model file from config. The exact reset conditions are: (1) process restart resets the counter; (2) a successful smoke probe (exit code 0) resets the counter to zero; (3) a successful `--model-id` override that produces exit code 0 resets the counter to zero; (4) any smoke exit code other than 13 does NOT reset the counter.
...
- **FR-017**: The system MUST rotate the audit log when it exceeds 5 MiB, retaining up to 5 rotated files (`audit.log`, `audit.log.1`, … `audit.log.5`). Each rotation creates a new file and copies current config snapshot (paths, ports, model names — no secrets) into the report directory alongside the audit log for post-mortem analysis.
- **FR-018**: When the hardware allowlist file at `~/.config/llm-runner/hardware-allowlist.json` is corrupt (invalid JSON, missing `fingerprint` or `pci_ids` keys, or unreadable), the system MUST fall back to treating the hardware as unknown: emit the same topology warning as FR-011, log a warning to stderr with the parse error reason, and create a fresh valid allowlist file on next successful hardware match. The session snooze file is NOT invalidated by allowlist corruption — it continues to suppress warnings for the current session.
...
- **FR-020**: The `smoke` command MUST support two output modes: (1) human-readable text printed to stdout with a per-slot summary and composite report footer, and (2) structured JSON via `--json` flag. The JSON document contains per-slot results (`slot_id`, `status`, `phase_reached`, `failure_phase`, `model_id`, `latency_ms`, `provenance`) and an `overall_exit_code` field. Enum definitions for typed fields:
...
_See `contracts/smoke-api.md` Section 4 (JSON Output Schema) for the complete JSON Schema Draft 2020-12 definition and detailed field descriptions._
...
- **Slot**: A GPU slot owning bind host, port, backend flavor, environment overlays, and at most one active model process. Identified by a slot ID (e.g., `arc_b580`, `rtx3090`). A slot transitions through the following operational states:
  ...
  A slot transitions to `degraded` when it is running but exhibits reduced health: probe latency exceeds a configurable threshold (default 10 s, see Appendix D), GPU telemetry reports errors but the process still responds, or the log buffer shows repeated error patterns (defined as 3 or more error-level log lines within a 60-second sliding window).
  A slot transitions from `degraded` back to `running` when **all** of the following hold for at least one consecutive health check cycle: probe latency is below the configurable threshold (`probe_latency_threshold_s`, default 10 s, see Appendix D), GPU telemetry reports no errors, and the log buffer shows fewer than 3 error-level log lines in the last 60 seconds. Recovery requires all three conditions — not just one — to prevent flapping between `degraded` and `running`.
...
- **AC-015**: Doctor and smoke use non-overlapping exit code families (doctor 1–9, smoke 10–19); each code maps to a specific, actionable outcome as defined in Appendix B.
...
> **Single source of truth**: This table is the authoritative definition of all exit codes. Any other documentation (including `contracts/smoke-api.md`) must reference this appendix rather than duplicating exit code definitions. When updating exit codes, always edit this table first.
...
| 10 | `smoke` | Server not ready (listen/accept timeout or connection refused) |
...
| 13 | `smoke` | Timeout (TCP or HTTP timeout) |
| 14 | `smoke` | Model not found (wrong `model` ID or empty models) |
