...
6. **Lockfiles** — Per-slot lockfiles with stale detection (5 min, psutil-based). Note: lockfile code (`check_lockfile_integrity`, `create_lock`, `release_lock`, `_clear_lockfile`) already exists in `src/llama_manager/process_manager.py` and needs integration updates to wire lockfile checks into the smoke and launch flows.
...
**Dependency Note**: `httpx` and `gguf` are planned additions to `pyproject.toml` under `[project.dependencies]` — they are not yet present. Verify they are added before implementation begins.
